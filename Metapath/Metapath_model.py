...
class Metapath:
    def __init__(self, num_nodes,n_sampled ,num_feat,enc, embedding_dim=200, lr=0.001):
        print(enc)
    ...
    def act_info(self):
        ...
        att = np.load('Metapath/main.py', allow_pickle=True)
        relation_dict = att.item()
        f2 = open('data/name2node_end.json', 'r', encoding='utf-8')
        ...
        for label in ['1','2','3','4','5','6']:
            for id in dictionary.keys():
                ...
                try:
                    id_code_new = self.enc.transform([id])
...
def get_graph():
    f = open('./data/Shareholder_r/stake.txt','r')
    data=[]
    for line in f:
        line = line[:-1]    
    ...
    print('Start giving id for nodes......')
    k=[]
    data_new = []
    data_new1 = []
    for i in data:
        if i['source'] not in k:
            k.append(i['source'])
        if i['target'] not in k:
            k.append(i['target'])
        if [i['source'],i['target'],i['value']] not in data_new:
            data_new.append([i['source'],i['target'],i['value']])
    for i in data_sys:
        if i['source'] not in k:
            k.append(i['source'])
        if i['target'] not in k:
            k.append(i['target'])
        if [i['source'],i['target'],i['value']] not in data_new1:
            data_new1.append([i['source'],i['target'],i['value']])
    enc=LabelEncoder()
    enc=enc.fit(k)
    output = open('data/system_all.pkl','wb')
    pickle.dump(enc,output)
    output.close
    print('Finished, start obtaining graph......')
    graph=[]
    for i in data_new1: 
        i[:2] = list(enc.transform(i[:2]))
        try:
            i[2]=float(re.findall(r"\d+\.?\d*",i[2])[0])/100
            graph.append(i[:3])
            print(i[:3])
        except:
            print(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--outputEmbedFile", type=str, default=f+'/embedding/EKGL.embed')
    args = parser.parse_args()

    graph,enc,len_node=get_graph()
    len_node = len_node +1
    G = nx.DiGraph()
    ...
    embedding_result = model_train(len_node,all_pairs,enc)